...
check "AGENTS.md not carried"         test ! -f "$target/AGENTS.md"
...
check "manifest ship-files excludes AGENTS.md"            bash -c "! grep -q '^AGENTS\\.md\$' '$manifest_ship_list'"
...
if [[ -d "$bootstrap_upstream/.git" ]]; then
  ...
  check "no AGENTS.md after upgrade"              test ! -f "$target/AGENTS.md"
