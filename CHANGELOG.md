...
behavior changes to `upgrade.sh`. No migration script ships with this
release — behavior changes take effect on the next run of the upgraded
`upgrade.sh`.

### Added

- **Antigravity Harness Adapter — FW-ADR-0026 (#338, #339, #342)**:
  `.agents/rules/team-contract.md` (`activation: always_on`) loads the
  full team contract into every Antigravity workspace conversation;
  `.agents/agents.md` stub registered. Cross-provider MCP
  non-primary-session rule (#289) ported into `CLAUDE.md` and `GEMINI.md`.
- **Antigravity Per-Role Roster — FW-ADR-0026 addendum (#346, Q-0033)**:
  16 `.agents/agents/<role>/agent.json` subagent definitions and 16
  `.agents/skills/<role>/SKILL.md` skill files, generated by
  `compile-runtime-agents.sh` (generator v0.3.0) and drift-checked by
  `lint-agent-contracts.sh`. The tech-lead subagent carries a
  halt-if-spawned guard.
- **Antigravity MCP Delegate Shim — FW-ADR-0027 (#343)**:
  `scripts/mcp/antigravity_delegate.py` — a Python-stdlib stdio MCP
  server exposing `antigravity_delegate(task, model?, timeout_s?)` so a
  Claude session can delegate tasks to Antigravity inline. Setup guide at
  `docs/mcp/antigravity-delegate-setup.md`.
- **Codacy Repository Config (#345)**: `.codacy.yaml` added — scopes
  the opengrep and lizard engines off
  `scripts/mcp/antigravity_delegate.py` to suppress confirmed false
  positives (no-shell `execvp`; reviewed complexity).

### Fixed

- **Upgrade Exec-Bit Preservation + YAML Intake-Log Indexing (#337)**:
  `upgrade.sh` `atomic_install` now preserves and repairs executable
  mode; `--verify` flags drift. `gen-register-index.sh` extended to
  index YAML-style intake logs alongside existing formats.
- **Multi-Version Upgrade Reliability — FW-ADR-0028 (#262)**: Trivial-delta
  auto-merge broadened to cover blank-line and comment-only deltas (beyond
  the prior SPDX-only scope). `--dry-run` now accurately previews
  auto-merge vs. conflict outcomes before a live run.

### Changed

- **Pre-Existing-Path Collision Default Changed to Take-Upstream (#262,
  FW-ADR-0028)**: When upstream newly ships a path that a downstream
  project already contains with no common baseline, `upgrade.sh` now
  takes the upstream version by default instead of prompting. Operators
  who relied on the previous always-prompt behavior must opt out using the
  new `--keep-local <path>` flag per run, or permanently via
  `.template-customizations`. Note: trivial-delta auto-merge discards
  cosmetic-only local additions in favor of upstream.

---

## v1.3.0 (2026-06-03)

Third MINOR release post v1.0.0 final. All changes are additive and
...
Second MINOR release post v1.0.0 final. All changes are additive.
...
**Key themes**:
- Upgrade-flow robustness: cluster-B version-check, cluster-C upgrade.sh
  follow-ups, cluster-E dogfood scrub paths, pre-release gate codified
  (PR #256, #255, #253, #237, #217, #209).
- Hook-behavior consolidation: cluster-A (inline SWDT_AGENT_PUSH, CUSTOMER_NOTES
  read-only paths, conflict tracking); cluster-G (model-routing rubric + CI
  enforcement via lint-agent-model-routing.sh).
- IEEE-paraphrase integration fan-out: #59 umbrella closed as wontfix; 8
  follow-ups (#238–#245) deferred per A-009 (1044/730/1012 integration,
