...
- [ ] T045 SC-007 fixture-downstream verification: set up a clean fixture at `v1.0.0-rc8`, run the post-burndown `scripts/upgrade.sh`, confirm no divergence (no multi-branch TEMPLATE_VERSION trap; settings.json receives new hook wires; version-check accurate; post-upgrade smoke-test passes).
