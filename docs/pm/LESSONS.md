...
- **OBS-G5-2 — duplicate routing tables**: resolved in `fix/cluster-G-model-routing` (issue #147 Part C). `## Role defaults` tier table replaced with a conceptual `## Class taxonomy` preface (no per-agent assignments). The `## Binding per-agent default-class table` is now the single canonical source with added Claude/OpenAI/Gemini equivalent columns and the `sme-template` row.
- **OBS-G5-3 — fallback-log.jsonl contract**: `scripts/log-fallback.sh` creates `docs/pm/fallback-log.jsonl` on first event. No file exists pre-event; this is intended steady-state. Create-on-first-write contract documented in `docs/pm/README.md` (issue #150, resolved).
