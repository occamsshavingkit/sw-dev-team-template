...
- **JIT file list:** <Mandatory list of files the assignee should load first. If no existing files are needed, write "none". To restrict reading scope and save tokens, specify line ranges via hash anchors (e.g., `path/to/file.py#L40-L80`) or specify compiled context files produced by the deterministic markdown compiler `llmdc` (e.g., `.claude/tmp/T-NNNN-context.llmd`).>
