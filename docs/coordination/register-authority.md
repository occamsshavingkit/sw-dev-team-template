...
**Register sharding (FW-ADR-0025).** Large registers (`CUSTOMER_NOTES.md`,
`OPEN_QUESTIONS.md`, `docs/pm/RISKS.md`, `docs/pm/LESSONS.md`) that
exceed ~150 KB are split into date-quarter shard files
(`<register>-YYYY-QN.md`) by `scripts/archive-registers.sh --quarter-roll`.
The active file (`<register>.md`) retains the current quarter's entries;
its canonical path is unchanged. A cross-shard index (`<register>-INDEX.md`)
is regenerated by `scripts/gen-register-index.sh` after each roll. The
authority table above refers to the active file; the shard and index files
carry the same authority for their respective quarters. See
`docs/agents/manual/librarian-manual.md` § "Archival mechanic" for the
full procedure.
