...
**Not shipped to downstream projects.** This template's own `FW-ADR-NNNN`
decision history (`docs/adr/fw-adr-NNNN-*.md`) is upstream
template-maintenance content, excluded from `scripts/scaffold.sh`'s
copy the same way `docs/audits/`, `docs/review/`, `docs/security/`,
`docs/proposals/`, and `docs/v2/` are — it records why the *template*
...
recommended sections may be omitted with a one-line rationale.
