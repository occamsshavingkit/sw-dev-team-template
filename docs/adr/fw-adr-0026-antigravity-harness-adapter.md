...
- [Links](#links)
- [Addendum — Per-role roster generation (Q-0033)](#addendum--per-role-roster-generation-q-0033)
  - [Confirmed facts (binary analysis + live probe, 2026-06-11 — HIGH confidence)](#confirmed-facts-binary-analysis--live-probe-2026-06-11--high-confidence)
  - [Surface decision: Option C — Both (agent.json subagents + SKILL.md skills)](#surface-decision-option-c--both-agentjson-subagents--skillmd-skills)
  - [Generated-file shapes](#generated-file-shapes)
  - [Generator changes (scripts/compile-runtime-agents.sh)](#generator-changes-scriptscompile-runtime-agentssh)
  - [Lint changes (scripts/lint-agent-contracts.sh)](#lint-changes-scriptslint-agent-contractssh)
  - [Five-surface sync table (updated)](#five-surface-sync-table-updated)
  - [Implementation change-set (Q-0033 scope)](#implementation-change-set-q-0033-scope)
  - [Open sub-points (confirm during/after implementation)](#open-sub-points-confirm-duringafter-implementation)
...
## Addendum — Per-role roster generation (Q-0033)

**Date:** 2026-06-11
**Status:** Accepted (extends original acceptance; no status change)
**Author:** `architect`
**Trigger:** Q-0033 (blocked implementation: "Antigravity operators cannot
autonomously select specialist roles"). Binary analysis of the installed
`agy` binary and a live probe (2026-06-11) resolved the skills-format
dispute noted in § 4 and supply the confirmed facts required before SE
implementation starts.

### Confirmed facts (binary analysis + live probe, 2026-06-11 — HIGH confidence)

1. **Skills surface.** `.agents/skills/<name>/SKILL.md` — one directory per
   skill, containing `SKILL.md` with YAML frontmatter. Optional
   subdirectories: `scripts/`, `examples/`, `resources/`, `references/`.
   Binary strings: "Must contain a SKILL.md file (instructions with YAML
   frontmatter)", "Location: skills/<skill_name>/". Live probe confirmed
   `agy` loads a workspace `.agents/skills/<n>/SKILL.md` as a skill.
   **Confidence: HIGH.**

2. **Subagents surface.** `.agents/agents/<name>/agent.json` with the
   `customAgent` wrapper (`name`, `systemPromptSections`, `toolNames`).
   Confirmed via protobuf string extraction (`ToolNames`/`toolNames`) and
   live probe that loaded a workspace `agent.json` as a custom subagent.
   **Confidence: HIGH.**

3. **Both surfaces load simultaneously.** Skills and subagents are
   independent; both are active when present. **Confidence: HIGH.**

**Dispute resolved (Q-0033).** Memory observation 12585 (2026-06-11)
reported skills use JSON path references rather than SKILL.md directories.
That reading is superseded by binary string-analysis and a live probe.
The correct format is SKILL.md directories per item 1 above.

### Surface decision: Option C — Both (agent.json subagents + SKILL.md skills)

**Chosen option: C — Both surfaces.**

Rationale (five points):

1. **Operator selection vs. auto-invocation are distinct channels.** Skills
   (`SKILL.md`) are description-matched and activated automatically when
   task context aligns — they serve capability discovery, not explicit
   dispatch. Subagents (`agent.json`) are the Antigravity mechanism an
   operator (or an orchestrating tech-lead session) uses to explicitly
   select and spawn a named specialist. The stated problem (operators
   cannot autonomously select specialist roles) is solved by subagents, not
   skills alone. Skills alone (PR #341's approach) address auto-invocation
   but not deterministic operator dispatch.

2. **Option A (subagents only) leaves a capability gap.** Without SKILL.md
   files, Antigravity has no description-matching path: a task that
   mentions "architecture decision" will not auto-route to the architect
   unless a skill entry exists. Subagents are explicitly selected; skills
   are semantically matched. Both channels serve different use cases and
   the cost of adding both is low (16 roles × 2 = 32 generated files, one
   generator pass).

3. **Consistency with PR #341 prior art.** PR #341 (branch
   `feat/antigravity-harness-adapter`, closed) generated 16 skills with the
   correct SKILL.md pattern and the `canonical_sha` drift mechanism.
   Discarding that work forces a rewrite. The correct path is to retain
   it and add the agent.json layer on top.

4. **Maintenance cost is bounded and machine-enforced.** Both surfaces are
   generated by `compile-runtime-agents.sh`; neither is hand-authored.
   `lint-agent-contracts.sh` will enforce `canonical_sha` on both. A single
   generator run keeps all surfaces synchronized. Maintenance delta between
   Option A and C is negligible (one additional generated-file template in
   the compiler).

5. **Tech-lead guard.** The `tech-lead` subagent entry requires a guard in
   its `systemPromptSections` content stating that if invoked as a subagent
   it should halt and report harness misconfiguration — because tech-lead
   is the main-session persona and must not be spawnable as a specialist
   (CLAUDE.md binding rule). This guard is mandatory for correctness; it
   is only possible with an agent.json entry.

### Generated-file shapes

#### Agent.json — `.agents/agents/<role>/agent.json`

```json
{
  "name": "<role-slug>",
  "description": "<verbatim description: field from .claude/agents/<role>.md>",
  "hidden": false,
  "config": {
    "customAgent": {
      "systemPromptSections": [
        {
          "title": "Role contract",
          "content": "Read `.claude/agents/<role>.md` (canonical role contract). Act only as that role. Return output in the role's required format."
        }
      ],
      "toolNames": [],
      "systemPromptConfig": {
...
**tech-lead guard.** For the `tech-lead` role only, the `content` field
of the sole `systemPromptSections` entry is replaced with:

```
This session has been invoked as an Antigravity subagent named tech-lead.
This is a harness misconfiguration: tech-lead is the main-session persona
and must not be spawned as a specialist subagent (binding rule per
.claude/agents/tech-lead.md and CLAUDE.md). Do not perform any work.
Report this condition to the operator and halt.
```

**`toolNames` mapping rule.** The `.claude/agents/<role>.md` frontmatter
`tools:` line lists Claude Code tool names. Antigravity's `toolNames`
field accepts tool identifiers in its own vocabulary. The mapping rule is
a **documented passthrough with a null default**: because Antigravity tool
names are not yet confirmed from a Tier-1 source, the generator emits
`"toolNames": []` (empty array — no restriction) for all roles. An empty
`toolNames` means the subagent inherits the session's tool context rather
than being restricted. This is the least-surprising safe default: no
tool is denied, no tool is granted beyond what the session already has.

When Antigravity tool-name vocabulary is confirmed (Tier-1 source), SE
extends the generator with a `ANTIGRAVITY_TOOLS_MAP` lookup table
(role-slug → `toolNames` array) in the same file as the Gemini model-class
table. Until then, the empty-array passthrough ships. A `WARN` is emitted
to stderr for each role with a non-empty `tools:` frontmatter line when
`ANTIGRAVITY_TOOLS_MAP` has no entry for that role.

**Drift mechanism.** agent.json files carry `canonical_sha` as a
top-level JSON string field alongside the `config` object:
...
```json
{
  ...
  "canonical_sha": "<SHA-1 of canonical file at generation time>",
  "generator": "scripts/compile-runtime-agents.sh",
  "generator_version": "<GENERATOR_VERSION at emit time>",
  "classification": "generated",
  "config": { ... }
}
```

`lint-agent-contracts.sh` reads `canonical_sha` from each agent.json and
compares it to `git rev-parse HEAD:.claude/agents/<role>.md`. Mismatch is
an error. This is the same mechanism used for `.opencode/agents/` and
`.gemini/agents/`.

#### SKILL.md — `.agents/skills/<role>/SKILL.md`
...
Body (identical to PR #341 pattern and existing `.gemini/agents/` + `.opencode/agents/` body):
...
**`trigger:` frontmatter field.** The confirmed SKILL.md schema accepts an
optional `trigger:` field. Its value for this purpose is the role's
`description:` field truncated to a natural-language noun phrase (e.g.,
`Software Architect` for the architect role). Adding `trigger:` is noted
as a low-value enhancement: `description:` already serves as the
description-matching signal Antigravity uses for skill selection. The
generator omits `trigger:` in the initial implementation; it can be added
per-role via the `ANTIGRAVITY_SKILL_TRIGGERS` lookup table in the generator
once the benefit of explicit triggers vs. description matching is confirmed
in a live session.

### Generator changes (`scripts/compile-runtime-agents.sh`)

**New flag:** `--no-antigravity-adapters` — skips `.agents/skills/` and
`.agents/agents/` output. Mirrors `--no-opencode-adapters` and
`--no-gemini-adapters`.

**New output targets (alongside the existing three):**

| Output path | Template | Source fields |
|---|---|---|
| `.agents/skills/<role>/SKILL.md` | SKILL.md shape above | `description:`, computed `canonical_sha` |
| `.agents/agents/<role>/agent.json` | agent.json shape above | `description:`, computed `canonical_sha`, tech-lead guard |

**`ANTIGRAVITY_OUT_DIR` variable:** `.agents` (root of both output trees).

**`GENERATOR_VERSION` bump:** `0.2.0` → `0.3.0` to mark the new output
targets. All previously generated files (`docs/runtime/agents/`,
`.opencode/agents/`, `.gemini/agents/`) are regenerated with the new
version string.

**Generation order within a role pass:**
1. Emit `.agents/skills/<role>/SKILL.md` (mkdir -p `.agents/skills/<role>`).
2. Emit `.agents/agents/<role>/agent.json` (mkdir -p `.agents/agents/<role>`).

**JSON emission.** The generator uses `printf` + here-doc with shell
variable interpolation — the same approach used for YAML frontmatter in
existing adapter targets. No external JSON tool required (POSIX sh
constraint retained).

### Lint changes (`scripts/lint-agent-contracts.sh`)

**New validated surface:** Antigravity adapters (both skills and agents).

**Checks added (run under `--generated-only` mode and default `all` mode):**

| Check | Failure condition |
|---|---|
| `canonical_sha` in each `SKILL.md` matches `git rev-parse HEAD:.claude/agents/<role>.md` | Mismatch → error |
| `canonical_sha` in each `agent.json` matches same | Mismatch → error |
| `description` field present and non-empty in `SKILL.md` | Absent or empty → error |
| `name` field present in `agent.json` | Absent → error |
| `classification: generated` present in `SKILL.md` | Absent → error |
| `classification` field present in `agent.json` with value `"generated"` | Absent or wrong → error |
| Stray files in `.agents/skills/` not matching `<role>/SKILL.md` pattern | Presence → warning |
| Stray files in `.agents/agents/` not matching `<role>/agent.json` pattern | Presence → warning |

**New `--mode` value:** `--antigravity-only` — validates only `.agents/`
surfaces. Mirrors `--generated-only`, `--canonical-only`, `--fixtures-only`.

**CRITICAL:** The new lint surface and the new generated files MUST be
committed together. A PR that modifies `.claude/agents/<role>.md` without
running `compile-runtime-agents.sh` (all targets, version 0.3.0) will fail
agent-contract CI because `canonical_sha` in all four adapter surfaces will
be stale.

### Five-surface sync table (updated)

| Surface | Path | Generator | Drift detection |
|---|---|---|---|
| Canonical | `.claude/agents/` | Hand-authored | `agent-contract.schema.json` + lint |
| Compact runtime | `docs/runtime/agents/` | `compile-runtime-agents.sh` | `canonical_sha` mismatch |
| OpenCode | `.opencode/agents/` | `compile-runtime-agents.sh` | `canonical_sha` mismatch |
| Gemini CLI | `.gemini/agents/` | `compile-runtime-agents.sh` | `canonical_sha` mismatch |
| Antigravity skills | `.agents/skills/` | `compile-runtime-agents.sh` | `canonical_sha` mismatch |
| Antigravity subagents | `.agents/agents/` | `compile-runtime-agents.sh` | `canonical_sha` mismatch |

### Implementation change-set (Q-0033 scope)

All work is `software-engineer` owned. No hand-authored files change.

| Artifact | Action |
|---|---|
| `scripts/compile-runtime-agents.sh` | Add `--no-antigravity-adapters` flag; add SKILL.md + agent.json emit targets; bump `GENERATOR_VERSION` to `0.3.0` |
| `scripts/lint-agent-contracts.sh` | Add `.agents/skills/` and `.agents/agents/` lint checks; add `--antigravity-only` mode |
| `.agents/skills/<role>/SKILL.md` ×16 | Generate via extended script (all 16 canonical roles) |
| `.agents/agents/<role>/agent.json` ×16 | Generate via extended script (all 16 canonical roles, tech-lead guard applied) |
| All existing generated surfaces | Regenerate with `GENERATOR_VERSION` 0.3.0 (byte-identical content except version stamp) |

**Live-validation step.** After the PR merges to main, the implementing SE
opens an Antigravity session on the scaffold and verifies: (a) the team
roster appears as selectable subagents, (b) a task phrased as "make an
architecture decision" auto-routes to the architect skill, and (c)
explicitly invoking `tech-lead` as a subagent returns the guard message
rather than starting an orchestration loop. Record pass/fail in the PR
comment. Failure reopens the PR and requires a corrective commit.

### Open sub-points (confirm during/after implementation)

1. **Rules-file ordering guarantee.** Only one rules file currently exists
   (`.agents/rules/team-contract.md`). Ordering across multiple rule files
   is not confirmed (LOW confidence per § "Findings"). Risk is low while
   the single-file constraint holds. If a second rules file is ever added,
   this must be resolved against a Tier-1 source before merging.

2. **`trigger:` frontmatter value.** Whether explicit `trigger:` fields in
   SKILL.md materially improve specialist selection beyond what
   `description:` matching already provides is unconfirmed. Confirm in a
   live Antigravity session post-implementation. If confirmed valuable,
   add `ANTIGRAVITY_SKILL_TRIGGERS` lookup table to the generator and
   regenerate.

3. **Antigravity tool-name vocabulary.** `toolNames` is emitted as `[]`
   (passthrough) pending vocabulary confirmation. Confirm from a Tier-1
   source (live WebFetch against Antigravity docs or binary string-analysis
   of the tool-name enumeration). When confirmed, add `ANTIGRAVITY_TOOLS_MAP`
   to the generator and regenerate.
