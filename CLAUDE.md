...
**MCP non-primary session carve-out (issue #289/#339).** A second exception applies when a Claude Code session is invoked via an MCP tool call from another session, not as the primary orchestrator. Such a session should suppress all orchestrator behavior (no spawning, no team-start prompt, no customer contact) and act directly as the role described in the MCP tool call preamble, defaulting to `software-engineer` if no role is specified. Detection: the session preamble indicates "Top-level tech-lead dispatched you", "You have already been spawned", or equivalent MCP framing. Return results directly in the tool response. See `AGENTS.md` § "MCP-connection / non-primary-session mode" (issue #289) for the full detection heuristic; `GEMINI.md` § "MCP Non-Primary Session" and `.agents/agents.md` § "MCP Non-Primary Session" carry the same rule on those harness surfaces (fw-adr-0026).
