...
10. In downstream projects, keep product work separate from framework
   ...
   task.
11. **Atomic customer questions (binding, strict reading).** Each customer-facing question MUST cover at most ONE decision axis (when a question is multi-axis, ask one axis per turn and queue the extras in `docs/OPEN_QUESTIONS.md`). A
